...
using System.Text.RegularExpressions;
  ...
  /// <summary>
    /// Summary description for ProjectFileHandler
    /// </summary>
public class ProjectFileHandler : IHttpHandler, IRequiresSessionState
    ...
    * ###################################*/
              ...
              && context.Request.Form["SectionId"] != null )
            {
                ...
                if (context.Request.Form["ProjectUsageId"] != "")
                {
                    ProjectUsageId = Convert.ToInt32(context.Request.Form["ProjectUsageId"].ToString());
                }
            ...
            try
            {
                ...
                UpdateTableDocs(context, TableName, Name, saveDirectory, SectionId, Description, PracticeId,ProjectUsageId);
        ...
        public void UpdateTableDocs(HttpContext context, string TableName, string Name, string saveDirectory, int SectionId, string Description, int PracticeId,int ProjectUsageId)
        {

            try
            {
                    file.SaveUploadData(TableName, Name, saveDirectory, System.DateTime.Now, SectionId, Convert.ToInt32(HttpContext.Current.Session["UserApplicationId"]), Description, "~/Themes/Images/GenericFileUploaderImage.png", ProjectUsageId);
            }
            catch (Exception exception)
            {
                throw exception;
