<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NCQAUploadedDocs.aspx.cs"
    Inherits="BMT.Webforms.NCQAUploadedDocs" %>
...
<head runat="server">
    ...
    <script type="text/javascript" src="<%= Page.ResolveClientUrl("~/Scripts/ncqaUploadedDocs.js") %>"></script>
</head>
<body>
    ...
    <asp:DataGrid ID="datagridDocViewer" runat="server" AllowPaging="true" AllowSorting="true"
        ...
        <Columns>
            <asp:BoundColumn DataField="FactorSequence" HeaderText="Factor" ItemStyle-HorizontalAlign="Left"
                HeaderStyle-Width="20" ItemStyle-BorderColor="#CCCCCC" ItemStyle-CssClass="grid-border" ReadOnly="true" SortExpression="FactorSequence" />
            <asp:BoundColumn DataField="Type" HeaderText="Type" ItemStyle-HorizontalAlign="Left" ItemStyle-CssClass="grid-border"
                HeaderStyle-Width="150" ItemStyle-BorderColor="#CCCCCC" ReadOnly="true" SortExpression="Type" />
            <asp:BoundColumn DataField="Title" HeaderText="Title" ItemStyle-HorizontalAlign="Left"
                HeaderStyle-Width="150" ItemStyle-BorderColor="#CCCCCC" ItemStyle-CssClass="grid-border" ReadOnly="true" SortExpression="Title" />
            <asp:BoundColumn DataField="DocLinkedTo" HeaderText="Linked to..." ItemStyle-CssClass="grid-border" ItemStyle-HorizontalAlign="Left"
                HeaderStyle-Width="50" ItemStyle-BorderColor="#CCCCCC" ReadOnly="true" SortExpression="DocLinkedTo" />
                    <asp:BoundColumn DataField="LastUpdatedDate" HeaderText="Last Uploaded" ItemStyle-CssClass="grid-border" ItemStyle-HorizontalAlign="Left"
                HeaderStyle-Width="50" ItemStyle-BorderColor="#CCCCCC" ReadOnly="true" DataFormatString="{0:MM/dd/yyyy}" SortExpression="LastUpdatedDate" />
            <asp:TemplateColumn HeaderText="Edit" ItemStyle-HorizontalAlign="Center" HeaderStyle-Width="30" ItemStyle-CssClass="grid-border"
                ItemStyle-BorderColor="#CCCCCC">
                <ItemTemplate>
                    <a onclick="javascript:FileMove('<%# DataBinder.Eval(Container.DataItem,"PCMHSequence") %>', '<%# DataBinder.Eval(Container.DataItem,"ElementSequence") %>', '<%# DataBinder.Eval(Container.DataItem,"FactorSequence") %>', '<%# DataBinder.Eval(Container.DataItem,"File") %>', '<%# DataBinder.Eval(Container.DataItem,"DocName") %>', '<%# DataBinder.Eval(Container.DataItem,"ReferencePage") %>', '<%# DataBinder.Eval(Container.DataItem,"RelevancyLevel") %>', '<%# DataBinder.Eval(Container.DataItem,"DocType") %>','<%# DataBinder.Eval(Container.DataItem,"FactorTitle") %>','<%# DataBinder.Eval(Container.DataItem,"ProjectId") %>');">
            ...
            <asp:TemplateColumn HeaderText="Replace" ItemStyle-HorizontalAlign="Center" HeaderStyle-Width="30"
                ItemStyle-BorderColor="#CCCCCC" ItemStyle-CssClass="grid-border">
                <ItemTemplate>
                    <a onclick="javascript:ProcessReplaceFile('<%# DataBinder.Eval(Container.DataItem,"DocName") %>', '<%# DataBinder.Eval(Container.DataItem,"ElementSequence") %>', '<%# DataBinder.Eval(Container.DataItem,"FactorSequence") %>', '<%# DataBinder.Eval(Container.DataItem,"PCMHSequence") %>', '<%# DataBinder.Eval(Container.DataItem,"ProjectId") %>', 'NCQA Submission', '<%# DataBinder.Eval(Container.DataItem,"PracticeId") %>', '<%# DataBinder.Eval(Container.DataItem,"SiteId") %>', '<%# DataBinder.Eval(Container.DataItem,"DocName") %>', '<%# DataBinder.Eval(Container.DataItem,"ReferencePage") %>', '<%# DataBinder.Eval(Container.DataItem,"RelevancyLevel") %>', '<%# DataBinder.Eval(Container.DataItem,"File") %>', '<%# DataBinder.Eval(Container.DataItem,"DocLinkedTo") %>','<%# DataBinder.Eval(Container.DataItem,"DocType") %>');">
            ...
            <asp:TemplateColumn HeaderText="Remove" ItemStyle-HorizontalAlign="Center" HeaderStyle-Width="30"
                ItemStyle-BorderColor="#CCCCCC" ItemStyle-CssClass="grid-border">
                <ItemTemplate>
                    <a onclick="javascript:ProcessDeleteFile('DeleteFiles.aspx?pcmh=<%# DataBinder.Eval(Container.DataItem,"PCMHSequence") %>&element=<%# DataBinder.Eval(Container.DataItem,"ElementSequence") %>&factor=<%# DataBinder.Eval(Container.DataItem,"FactorSequence") %>&file=<%#  DataBinder.Eval(Container.DataItem,"File") + "|" + DataBinder.Eval(Container.DataItem,"DocName") + "|" %>&project=<%# DataBinder.Eval(Container.DataItem,"ProjectId")%>&practiceId=<%# DataBinder.Eval(Container.DataItem,"PracticeId") %>&siteId=<%# DataBinder.Eval(Container.DataItem,"SiteId") %>&pageNo=2','<%# DataBinder.Eval(Container.DataItem,"PCMHSequence") %>','<%# DataBinder.Eval(Container.DataItem,"ElementSequence") %>','<%# DataBinder.Eval(Container.DataItem,"FactorSequence") %>','<%# DataBinder.Eval(Container.DataItem,"DocType") %>','<%# DataBinder.Eval(Container.DataItem,"FactorTitle") %>','<%# DataBinder.Eval(Container.DataItem,"DocLinkedTo") %>');">
                ...
                <td>
                    <asp:Panel ID="pnlDeleteFile" runat="server">
                        <iframe scrolling="no" id="iFrameFileDelete" style="overflow: hidden; width: 100%;
                            min-height: 300px; margin: 0px;" src="DeleteFiles.aspx" frameborder="0"></iframe>
                    </asp:Panel>
                </td>
            </tr>
        </table>
