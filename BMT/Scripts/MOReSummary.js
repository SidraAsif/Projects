...
$('#chbReviewed').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "../WebServices/NCQAService.asmx/SaveMOReStatus",
        data: "{'_element':'Reviewed','_status':'" + status + "','_projectUsageId':'" + $('#hdnSummaryProjectUsageId').val() + "','siteId':'" + $('#hdnSiteId').val() + "','_level':'" + $('#hdnLevel').val() + "','_templateId':'" + $('#hdnTemplateId').val() + "'}",
...
$('#chbSubmitted').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "../WebServices/NCQAService.asmx/SaveMOReStatus",
        data: "{'_element':'Submitted','_status':'" + status + "','_projectUsageId':'" + $('#hdnSummaryProjectUsageId').val() + "','siteId':'" + $('#hdnSiteId').val() + "','_level':'" + $('#hdnLevel').val() + "','_templateId':'" + $('#hdnTemplateId').val() + "'}",
        contentType: "application/json; charset=utf-8",
        beforeSend: function () {
        },
        complete: function () {
...
$('#chbRecognized').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "../WebServices/NCQAService.asmx/SaveMOReStatus",
        data: "{'_element':'Recognized','_status':'" + status + "','_projectUsageId':'" + $('#hdnSummaryProjectUsageId').val() + "','siteId':'" + $('#hdnSiteId').val() + "','_level':'" + $('#hdnLevel').val() + "','_templateId':'" + $('#hdnTemplateId').val() + "'}",
        contentType: "application/json; charset=utf-8",
        beforeSend: function () {
        },
        complete: function () {
...
$('#btnSubmitNCQA').live('click', function () {
    ...
    if (isRequired == 'false') {
        if ($("#btnSaveAPICredentials")[0]) {
            $("#btnSaveAPICredentials").click();
            $('#lightbox, .NCQACredentials-popup').fadeOut(300);
            $('#btnRequestSubmission').attr('disabled', true);
        }
    }
...
//MOReCorporate Submission Popup

function DisplayMOReCorporateElement() {
    $('#lightbox, .MOReCorporateElement-popup').fadeIn(300);
    var browser = navigator.appName;
    var practiceId = $('#hiddenPracticeId').val();
    var siteId = $('#hiddenSiteId').val();
    var templateId = $('#hiddentemplateId').val();
    $('#IframeDocViewer').attr('src', '../Webforms/MOReCorporateSubmission.aspx?practiceId=' + practiceId + '&SiteId=' + siteId + '&TemplateId=' + templateId);
    // Remove td to fix close[x] tag wrapping in IE
    if (browser != "Microsoft Internet Explorer") {
        $('.MOReCorporateElement-popup').css('top', '0%');
        $('.MOReCorporateElement-popup').css('top', '0%');
    }
}

$('a#close-MOReCorporateElement').live('click', function () {
    $('#lightbox, .MOReCorporateElement-popup').fadeOut(300);
});
